...
int reopen_socket(const char *host, uint16_t port) {
   int sock = 0;
      ...
      //----------------------------------------------------------------------------
      //DONE: Implement a loop that attempts a certain number of times to open a 
...
int submit_request(int sock, const char *host, uint16_t port, char *resource){
   ...
   if (sent_bytes < 0){
      //----------------------------------------------------------------------------
      //DONE:  Reimplement the retry logic to reopen the socket
      //
   ...
   //--------------------------------------------------------------------------------
   // DONE:  Make sure you understand the calculations below
   //
   ...
   // from the server
   //
   // YOUR ANSWER:  We need to know where the payload starts which will hold the body of the http response, it starts at
   // the offset of header_len into the data, so intial_data is how much data there is left as the actual
   // payload after the header_len bytes.
   //
   // We also need to know how much more data we need to recv which will be until we grab content_len bytes. Since the
   // Content-Length tells us how much body data there is total, and inital_data is how much of the body we've already
   // grbbed, the remaining bytes to recv is content-length - initial_data
   // 
   //
   //--------------------------------------------------------------------------------
   ...
   while(bytes_remaining > 0){
      //-----------------------------------------------------------------------------
      // DONE:  Continue receiving data from the server
      //
      ...
      //-----------------------------------------------------------------------------
      if((bytes_recvd = recv(sock,recv_buff,sizeof(recv_buff),0))<0){
         close(sock);
         return -1;
      }
   ...
   //
   // YOUR ANSWER:  Since we are using Connection:Keep-Alive we try to reuse the same socket to reduce the overhead of
   // setting up the socket and new connection (the 8 or so TLS messages and the TCP handshake etc). We can return the
   // socket and if the host doesn't close the connection we can use it on the next request, otherwise we can detect if
   // the host closed the connection because the next send will return -1
   //
   //--------------------------------------------------------------------------------
   return sock;
}
